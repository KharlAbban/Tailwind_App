...
const Breadcrumb = () => {
  ...
  const breadcrumb_elements = pathsArray.map((path, index) => {
    ...
    return <li className="inline-flex items-center truncate" key={path}>
            <a className="capitalize flex items-center hover:text-blue-600 focus:outline-none focus:text-blue-600" href={linkTo}>
            {path}
            </a>
  ...
  return (
    <ol className="text-sm flex items-center whitespace-nowrap text-gray-500 font-semibold" aria-label="Breadcrumb">
        {breadcrumb_elements}
    </ol>
  )
}
