  ...
  return (
    <aside id="dashboardSibebar" className={`fixed top-0 left-0 z-45 w-64 max-md:-translate-x-full md:translate-x-0 max-lg:w-16 transition-transform h-screen bg-deepBlue pt-20 ${sidebarVisibility ? 'max-md:transform-none max-md:w-64': ''}`}>
      ...
      <ul class="space-y-2 font-medium">
         <li>
            <a href="#" class="flex gap-2 w-full items-center p-2 text-white hover:bg-blue-900">
               <MdDashboard className="text-3xl ml-2 text-gray-300" />
               <span class={`ms-3 lg:inline-block text-xl ${sidebarVisibility ? '' : 'hidden'}`}>Dashboard</span>
            </a>
         </li>
         <li>
            <a href="#" class="flex gap-2 w-full items-center p-2 text-white hover:bg-blue-900">
               <FaFile className="text-3xl ml-2 text-gray-300" />
               <span class={`ms-3 lg:inline-block text-xl ${sidebarVisibility ? '' : 'hidden'}`}>Tickets</span>
            </a>
         </li>
         <li>
            <a href="#" class="flex gap-2 w-full items-center p-2 text-white hover:bg-blue-900">
               <FaRegChartBar className="text-3xl ml-2 text-gray-300" />
               <span class={`ms-3 lg:inline-block text-xl ${sidebarVisibility ? '' : 'hidden'}`}>Statistics</span>
            </a>
         </li>
         <li>
            <a href="#" class="flex gap-2 w-full items-center p-2 text-white hover:bg-blue-900">
               <IoSettings className="text-3xl ml-2 text-gray-300" />
               <span class={`ms-3 lg:inline-block text-xl ${sidebarVisibility ? '' : 'hidden'}`}>Settings</span>
            </a>
         </li>
      </ul>
